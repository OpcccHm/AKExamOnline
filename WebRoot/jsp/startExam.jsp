...
function addRows() {
	//创建第一个td
	var $tdClass = $("<td>"+
			"<input type='hidden' value='"+${param.pid }+"'>"+
			"<select>"+
	...
	var $tdDate = $("<td>"+
			...
			"<input type='text' class='Wdate' onfocus='WdatePicker({skin:\"whyGreen\",dateFmt: \"yyyy-MM-dd HH:mm\",lang:\"zh-cn\", minDate: \"%y-%M-%d 08:00}\" })'/>"+
			"</td>");
	//创建第三个td
	var $tdDelete = $("<td>"+
...
function updateTr() {
	$("tr").each(function(index,element){
		$($(":hidden").get(index)).attr("name","paperClasses["+index+"].pid");
	...
	<form action="paper/paper_startExam" method="post">
		<div align="center">
			<input type="submit" value="开始考试"> <input type="reset"
				value="取消"> <br> <font>请选择参加考试的班级及开考时间${param.pid }</font>
		</div>
		<table id="tab_exam" align="center" cellspacing="1px" >
			...
			<tr>
				<td>
					<input type="hidden" name="paperClasses[0].pid" value="${param.pid }">
